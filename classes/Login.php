...
class Login extends DBConnection {
	...
	public function index(){
		echo "<h1>Рұқсат жоқ</h1> <a href='".base_url."'>Артқа.</a>";
	}
	...
	function client_login(){
		...
		if($this->conn->error){
			$resp['status'] = 'failed';
			$resp['msg'] = "Деректерді алу кезінде қате орын алды. Қате:". $this->conn->error;
		}else{
		if($qry->num_rows > 0){
			$res = $qry->fetch_array();
			if($res['status'] == 1){
				...
				$resp['status'] = 'failed';
				$resp['msg'] = "Есептік жазбаңыз әлі расталмаған.";
			}
			
		}else{
		$resp['status'] = 'failed';
		$resp['msg'] = "Жарамсыз электрондық пошта немесе құпия сөз.";
		}
		}

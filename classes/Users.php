...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		if(empty($id)){
			...
			if($qry){
				...
				$this->settings->set_flashdata('success','Пайдаланушы мәліметтері сәтті сақталды.');
				$resp['status'] = 1;
			}else{
				$resp['status'] = 2;
			}
			...
			if($qry){
				$this->settings->set_flashdata('success','Пайдаланушы мәліметтері сәтті жаңартылды.');
		...
		if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
			...
			if(!in_array($type,$allowed)){
				$resp['msg'].=" Бірақ сурет жарамсыз файл түріне байланысты жүктеп салынбады.";
			}else{
				$new_height = 200; 
				$new_width = 200; 
				...
				$resp['msg'].=" Бірақ сурет белгісіз себепке байланысты жүктелмеді.";
				}
			}
	...
	public function delete_users(){
		...
		if($qry){
			...
			$this->settings->set_flashdata('success','Пайдаланушы мәліметтері сәтті жойылды.');
	...
	public function verify_client(){
		...
		if($update){
			$this->settings->set_flashdata('success','клиент тіркелгісі сәтті расталды.');
			$resp['status'] = 'success';
		}else{
			$resp['status'] = 'failed';
		}

...
<div class="container-fluid">
    <dl>
        <dt class="text-muted">Қызмет</dt>
        <dd class='pl-4 fs-4 fw-bold'><?= isset($name) ? $name : '' ?></dd>
        <dt class="text-muted">Қызмет көрсету:</dt>
        <dd class='pl-4'>
            <p class=""><small><?= isset($for) ? ($for) : '' ?></small></p>
        </dd>
        <dt class="text-muted">Сипаттама</dt>
        <dd class='pl-4'>
            <p class=""><small><?= isset($description) ? html_entity_decode($description) : '' ?></small></p>
        </dd>
        <dt class="text-muted">Алым</dt>
        <dd class='pl-4 fs-4 fw-bold'><?= isset($fee) ? number_format($fee,2) : '0.00' ?></dd>
    </dl>
    <div class="col-12 text-right">
        <button class="btn btn-flat btn-sm btn-dark" type="button" data-dismiss="modal"><i class="fa fa-times"></i> Жабу</button>
    </div>
</div>
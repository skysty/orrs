            ...
            <div class="card card-outline card-primary rounded-0 shadow">
                <div class="card-body">
                    ...
                    <div class="callout border-primary rounded-0 shadow">
                        <fieldset>
                            <legend class="text-muted">Жол жүру билетін табу</legend>
                            <form action="" id="filter-schedule">
                                <div class="row align-items-end">
                                    <div class="col-md-3 col-sm-4">
                                        <label for="date" class="control-label">Қалаған күніңіз</label>
                                        <input type="date" name="date" id="date" class="form-control form-control-sm rounded-0" value="<?= $date ?>" required>
                                    </div>
                                    <div class="col-md-3 col-sm-4">
                                        <label for="time" class="control-label">Қалаған уақытыңыз</label>
                                        <input type="time" name="time" id="time" class="form-control form-control-sm rounded-0" value="<?= $time ?>" required>
                                    </div>
                                    <div class="col-md-3 col-sm-4">
                                        <button class="btn btn-flat btn-primary"><i class="fa fa-search"></i> Іздеу</button>
                                    </div>
                                </div>
                            </form>
                        </fieldset>
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td class="px-0">
                                        <?php if($row['type'] == 1): ?>
                                        <div class="px-1 border-bottom"><span class="text-muted fa fa-calendar"></span> Күнделікті</div>
                                        <?php else: ?>
                                    ...
                                    <td class="px-0">
                                        <div class="px-1 border-bottom"><span class="text-muted">Кайдан:</span> <b><?= $row['route_from'] ?></b></div>
                                    ...
                                    <td class="px-0">
                                        <div class="px-1 border-bottom"><span class="text-muted">Купе:</span> <span class="text-muted fa fa-user"></span> <b><?= $row['type'] == 1 ? "<i class='fa fa-question' title='Слот сіз қалаған күнге байланысты.'></i>" : number_format($fc_slot) ?></b> <span class="text-muted ml-2 fa fa-tag"></span> <b><?= rtrim(number_format($row['first_class_fare'],2),'.') ?></b></div>
                                        <div class="px-1"><span class="text-muted">Пласкарт</span> <span class="text-muted fa fa-user"></span> <b><?= $row['type'] == 1 ? "<i class='fa fa-question' title='Слот сіз қалаған күнге байланысты.'></i>" : number_format($e_slot) ?></b> <span class="text-muted ml-2 fa fa-tag"></span> <b><?= rtrim(number_format($row['economy_fare'],2),'.') ?></b></div>
                                    </td>
                                    <td class="px-1" align="center">
                                        <a href="<?php echo base_url ?>admin/?page=reserveticket&sid=<?= $row['id'] ?>" class="btn btn-flat btn-primary btn-sm" >Брондау <i class="fa fa-angle-right"></i></a>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
                        </tbody>
...
<script>
    $(function(){
        $('#filter-schedule').submit(function(e){
            e.preventDefault();
            location.href = "<?php echo base_url ?>admin/?page=buyticket&"+$(this).serialize();
        })
    })
</script>